...
class MetadataEngine:
    ...
    def translate_table_name(self, chinese_name: str, module_name: str = "", table_type: str = "master") -> str:
        ...
        # 2. 识别核心实体（按优先级）
        core_entity = "data"  # 默认值
        for chinese_entity in CORE_ENTITY_PRIORITY:
            if chinese_entity in cleaned_name:
                # 提取核心实体对应的英文
                for cn, en in CORE_ENTITY_DICT.items():
                    if cn in chinese_entity:
                        core_entity = en
                        break
                break
        
        # 3. 处理业务对象部分 - 优先匹配最长的业务对象
        translated_business = "info"  # 默认值
        for full_business_obj in sorted(BUSINESS_OBJECT_DICT.keys(), key=len, reverse=True):
            if full_business_obj in cleaned_name:
                translated_business = BUSINESS_OBJECT_DICT[full_business_obj]
                break
        
        # 4. 组合格式化
        if table_type == "dictionary" or any(keyword in cleaned_name for keyword in ["字典", "类型", "层次"]):
            # 为字典表添加特殊前缀
            english_name = f"{module_prefix}dict_{core_entity}_{translated_business}".lower()
        else:
            english_name = f"{module_prefix}{core_entity}_{translated_business}".lower()
