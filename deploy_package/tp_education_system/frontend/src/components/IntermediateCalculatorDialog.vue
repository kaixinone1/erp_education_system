...
const loadData = async () => {
  ...
  loading.value = true
  try {
    const response = await fetch(`/api/intermediate/${props.tableName}/detail/${props.teacherId}`)
...
const handleCalculate = async () => {
  ...
  try {
    const response = await fetch(`/api/intermediate/${props.tableName}/calculate/${props.teacherId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        custom_retirement_date: result.actual_retirement_date || null
      })
    })
...
const handleSave = async () => {
  ...
  saving.value = true
  try {
    const response = await fetch(`/api/intermediate/${props.tableName}/save-retirement-calculation/${props.teacherId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
