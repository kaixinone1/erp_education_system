  ...
  <div class="tag-relations-container">
    ...
    >
      ...
      <div v-if="selectedTeacherId" class="tag-selection">
        ...
        <div v-if="tagCategories.length === 0" style="text-align: center; padding: 20px; color: #909399;">
        ...
        <div v-if="tagCategories.length === 0 && !loading" class="no-tags">暂无可用标签</div>
      </div>
...
import { Search, Refresh, Plus, Delete, Loading } from '@element-plus/icons-vue'
...
const teacherInfo = computed(() => {
  return selectedTeacherInfo.value
})
...
const fetchTagCategories = async () => {
  try {
    const response = await fetch('/api/tag-relations/categories')
    if (response.ok) {
      const result = await response.json()
