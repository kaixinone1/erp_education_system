...
# 通用模板系统 - 搜索教师API
@app.get("/api/template/search-teachers")
async def template_search_teachers(keyword: str = "", search_type: str = "姓名"):
    """通用模板填报 - 搜索教师"""
    try:
        ...
        if search_type == "姓名":
            cursor.execute("""
                SELECT id, "姓名", "身份证号码", "性别", "出生日期", "民族",
                       "籍贯", "参加工作日期", "联系电话", "档案出生日期"
            ...
            cursor.execute("""
                SELECT id, "姓名", "身份证号码", "性别", "出生日期", "民族",
                       "籍贯", "参加工作日期", "联系电话", "档案出生日期"
                FROM teacher_basic_info
                WHERE "姓名" LIKE %s OR CAST(id AS TEXT) LIKE %s
                ORDER BY id
            """, (f"%{keyword}%", f"%{keyword}%"))
        ...
        for row in rows:
            teachers.append({
                ...
                "id_card": row[2] or "",
                "gender": row[3] or "",
                "birth_date": str(row[4]) if row[4] else "",
                "ethnicity": row[5] or "",
                "native_place": row[6] or "",
                "work_start_date": str(row[7]) if row[7] else "",
                "contact_phone": row[8] or "",
                "archive_birth_date": str(row[9]) if row[9] else ""
            })
        ...
        return {"success": False, "teachers": [], "error": str(e)}
