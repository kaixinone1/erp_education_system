  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.modules && data.modules.length > 0) {
        // 根据用户角色过滤导航项：非县级管理员隐藏 requireAdmin 的节点
        const isAdmin = userStore.是县级管理员
        const filterAdminItems = (items: any[]) => {
          return items
            .filter((item: any) => {
              // 如果标记了 requireAdmin 且当前用户不是县级管理员，隐藏
            ...
            // 过滤掉 children 为空数组的模块
            .filter((item: any) => {
              if (item.children && item.children.length === 0) return false
              return true
            })
        }
        navigationModules.value = filterAdminItems(data.modules)
      } else {
