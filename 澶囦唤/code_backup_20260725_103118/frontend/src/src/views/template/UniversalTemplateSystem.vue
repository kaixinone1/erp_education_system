            ...
            <el-dropdown style="margin-left: 6px" trigger="click">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item command="excel" @click="exportTemplate(scope.row)" :disabled="scope.row.原始文件 && scope.row.原始文件.toLowerCase().endsWith('.docx')">
                    Excel格式
                  </el-dropdown-item>
                  <el-dropdown-item command="word" @click="exportWord(scope.row)" :disabled="!scope.row.原始文件 || !scope.row.原始文件.toLowerCase().endsWith('.docx')">
                    Word格式
                  </el-dropdown-item>
                  <el-dropdown-item command="pdf" @click="exportPdf(scope.row)">
                    PDF格式
                  </el-dropdown-item>
      ...
      <el-table v-else :data="fileSelectRecords" style="width:100%" max-height="350">
        ...
        <el-table-column label="操作" width="280">
          <template #default="scope">
            ...
            <el-button v-if="fileSelectMode === 'print'" size="small" type="primary" @click="downloadSelectedFile(scope.row, 'Print')" :disabled="!scope.row.有HTML">
              打印
            </el-button>
          </template>
        </el-table-column>
...
async function previewTemplate(row) {
  try {
    ...
    if (templateType === 'word') {
      ...
      if (container) {
        container.innerHTML = '<div style="text-align:center;padding:60px 20px;color:#909399;"><p style="font-size:16px;margin-bottom:10px;">Word模板不支持在线预览</p><p style="font-size:14px;">请点击「填报」按钮选择职工并生成Word和PDF文件</p></div>'
      }
      ElMessage.info('Word模板不支持在线预览，请使用填报功能')
      return
    }
...
async function exportWord(row) {
  await openFileSelectDialog(row, 'word')
}
