...
def get_id_card_column(table_name: str, cursor=None) -> Optional[str]:
    """自动查找表中身份证号码对应的实际数据库列名"""
    # 候选列名列表（按优先级排序）
    candidates = ['id_card', '身份证号码']
    ...
    try:
        with open(schema_file, 'r', encoding='utf-8') as f:
            ...
            for field in table_fields:
                en = field.get('english_name') or field.get('targetField', '')
                sf = field.get('sourceField', '')
                if en == 'id_card' or sf == '身份证号码':
...
@router.get("/tables")
def get_tables():
    """获取所有表列表（与表结构管理格式一致：中文名+英文名，支持搜索）"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        table_name_mappings = load_table_mappings()
        reverse_mappings = table_name_mappings.get("reverse_mappings", {})
        ...
        for row in cursor.fetchall():
            ...
            # 优先从 reverse_mappings 查找
            if table_name in reverse_mappings:
                mapped_value = reverse_mappings[table_name]
                if isinstance(mapped_value, str):
                    chinese_name = mapped_value
                    has_chinese_name = True
                elif isinstance(mapped_value, dict):
                    chinese_name = mapped_value.get("chinese_name", table_name)
                    has_chinese_name = True
            ...
            if not has_chinese_name:
                if row[1]:
                    ...
                    has_chinese_name = chinese_name != table_name
...
def get_dict_values(req: dict):
    ...
    try:
        ...
        # 优先：加载 merged_schema_mappings.json 获取字典关联配置
        schema_file = os.path.join(os.path.dirname(__file__), '..', 'config', 'merged_schema_mappings.json')
        try:
            with open(schema_file, 'r', encoding='utf-8') as f:
                schema_data = json.load(f)
                    ...
                    if target == field_name:
                        ...
                        if relation_table:
                            ...
                            if len(cols) >= 2:
                                ...
                                values = [{"value": row[0], "label": str(row[1])} for row in cursor.fetchall()]
                                cursor.close()
                                conn.close()
                                return {"status": "success", "values": values, "source": "dict"}
                            cursor.close()
                            conn.close()
        except Exception as e:
            print(f"加载字典配置失败: {e}")
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute(f'SELECT DISTINCT "{field_name}" FROM "{table_name}" WHERE "{field_name}" IS NOT NULL ORDER BY "{field_name}"')
            rows = cursor.fetchall()
            values = [{"value": row[0], "label": str(row[0])} for row in rows]
            cursor.close()
            conn.close()
            return {"status": "success", "values": values, "source": "data"}
        except Exception as e:
            print(f"从数据表查询去重值失败: {e}")
            return {"status": "success", "values": []}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def get_table_fields(req: TableFieldsRequest):
    ...
    try:
        ...
        # 加载 merged_schema_mappings.json 获取中文字段名映射和字典关联
        schema_file = os.path.join(os.path.dirname(__file__), '..', 'config', 'merged_schema_mappings.json')
        chinese_labels = {}
        dict_field_map = {}  # {column_name: relation_table}
        try:
            with open(schema_file, 'r', encoding='utf-8') as f:
                schema_data = json.load(f)
                ...
                for field in table_fields:
                    ...
                    # 记录字典关联
                    relation_table = field.get('relation_table', '')
                    if relation_table and english_name:
                        dict_field_map[english_name] = relation_table
        except Exception as e:
            print(f"加载 merged_schema_mappings.json 失败: {e}")

        columns = cursor.fetchall()

        # 动态分析每个字段：查询去重值数量，判断是否有多选项
        fields = []
        for row in columns:
            column_name = row[0]
            data_type = row[1]
            ...
            # 跳过明显无意义的字段
            skip_types = ('timestamp without time zone', 'timestamp with time zone', 'bytea')
            if data_type in skip_types or column_name in ('id', 'created_at', 'updated_at'):
                fields.append({
                    "name": column_name,
                    "label": chinese_name,
                    "type": data_type,
            ...
            # 检查字典关联
            has_dict = column_name in dict_field_map
            source = "dict" if has_dict else ""
            ...
            if not has_dict:
                try:
                    cursor2 = conn.cursor()
                    cursor2.execute(f'SELECT COUNT(DISTINCT "{column_name}") FROM "{req.table_name}" WHERE "{column_name}" IS NOT NULL')
                    distinct_count = cursor2.fetchone()[0] or 0
                    cursor2.close()
                    # 如果去重值在 2-50 之间，视为有"多个选项"
...
def aggregate_query(req: AggregateQueryRequest):
    ...
    try:
        ...
        if len(req.tables) == 1:
            ...
            if req.tags:
                id_card_col = get_id_card_column(table_name, cursor)
                if id_card_col:
                    teacher_id_card = get_id_card_column('teacher_basic_info', cursor) or "id_card"
                    placeholders = ','.join(['%s'] * len(req.tags))
                    tag_count = len(req.tags)
                    tag_filter = f"""
                        "{id_card_col}" IN (
                            SELECT "{teacher_id_card}" FROM teacher_basic_info
                            WHERE id IN (
            ...
            if req.filters:
                for f in req.filters:
                    ...
                    # 过滤条件只应用于对应表，跳过不属于当前表的条件
                    if field_table and field_table != table_name:
                        continue
                    ...
                    if field_name:
                        if field_values and len(field_values) > 0:
                            ...
                            # 统一转为字符串，避免字典表整数值与varchar列类型不匹配
                            params.extend([str(v) for v in field_values])
                        elif field_value:
            ...
            for idx, table_info in enumerate(req.tables[1:], 1):
                ...
                # 自动查找两个表的身份证号码列名
                first_id_card = get_id_card_column(first_table_name, cursor) or "id_card"
                join_id_card = get_id_card_column(table_name, cursor) or "id_card"
                join_parts.append(f'LEFT JOIN "{table_name}" {table_alias} ON "{first_table_name}"."{first_id_card}" = {table_alias}."{join_id_card}"')
            ...
            if req.tags:
                ...
                tag_count = len(req.tags)
                tag_filter = f"""
                    "{first_table_name}"."{first_id_card}" IN (
                        SELECT "{teacher_id_card}" FROM teacher_basic_info
                        WHERE id IN (
                            SELECT employee_id FROM employee_tag_relations
                            WHERE tag_id IN (
