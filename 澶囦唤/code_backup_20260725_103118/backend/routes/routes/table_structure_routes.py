...
async def get_all_tables():
    ...
    try:
        ...
        # 从 table_name_mappings.json 获取中文表名
        table_name_mappings = read_json_file(TABLE_NAME_MAPPINGS_FILE)
        mappings = table_name_mappings.get("mappings", {})
        
        table_info = []
        reverse_mappings = table_name_mappings.get("reverse_mappings", {})
        
        for table_name in tables:
            # 查找中文表名 - 优先使用 reverse_mappings
            chinese_name = None
            table_type = "master"
            has_chinese_name = False
            
            # 先从 reverse_mappings 查找
            if table_name in reverse_mappings:
                ...
                if isinstance(mapped_value, str):
                    ...
                    if chinese_name in mappings:
                        table_type = mappings[chinese_name].get("table_type", "master")
                elif isinstance(mapped_value, dict):
                    chinese_name = mapped_value.get("chinese_name", table_name)
                    table_type = mapped_value.get("table_type", "master")
                    has_chinese_name = True
            
            # 如果没找到，再从 mappings 查找
            if not has_chinese_name:
                for cn_name, info in mappings.items():
                    if info.get("english_name") == table_name:
            ...
            table_info.append({
                "name": table_name,
                "chinese_name": chinese_name or table_name,
...
async def add_field(table_name: str, data: Dict[str, Any]):
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # 构建字段类型（列名用双引号包裹以支持含空格/特殊字符的列名）
            quoted_name = f'"{field_name}"'
            if field_type == "VARCHAR":
                length = data.get("length", 255)
                column_def = f"{quoted_name} VARCHAR({length})"
            elif field_type == "INTEGER":
                column_def = f"{quoted_name} INTEGER"
            elif field_type == "DECIMAL":
                column_def = f"{quoted_name} DECIMAL(10,2)"
            elif field_type == "DATE":
                column_def = f"{quoted_name} DATE"
            elif field_type == "DATETIME":
                column_def = f"{quoted_name} TIMESTAMP"
            elif field_type == "BOOLEAN":
                column_def = f"{quoted_name} BOOLEAN"
            elif field_type == "TEXT":
                column_def = f"{quoted_name} TEXT"
            else:
                column_def = f"{quoted_name} VARCHAR(255)"
...
@router.get("/mappings")
async def get_table_name_mappings():
    """获取表名映射关系（中文表名 <-> 英文表名）"""
    try:
        table_name_mappings = read_json_file(TABLE_NAME_MAPPINGS_FILE)
        ...
        print(f"获取表名映射失败: {e}")
        raise HTTPException(status_code=500, detail=f"获取表名映射失败: {str(e)}")


@router.post("/mappings")
async def save_table_name_mapping(data: Dict[str, Any]):
    """保存表名映射关系"""
    try:
        ...
        if not chinese_name or not english_name:
            raise HTTPException(status_code=400, detail="中文表名和英文表名都不能为空")

        # 读取现有映射
        table_name_mappings = read_json_file(TABLE_NAME_MAPPINGS_FILE)
        if "mappings" not in table_name_mappings:
            table_name_mappings["mappings"] = {}
        if "reverse_mappings" not in table_name_mappings:
            table_name_mappings["reverse_mappings"] = {}

        # 保存映射关系
        table_name_mappings["mappings"][chinese_name] = {
            "english_name": english_name,
            "table_type": table_type,
            "created_at": datetime.now().isoformat()
        }
        table_name_mappings["reverse_mappings"][english_name] = chinese_name

        # 写入文件
        if write_json_file(TABLE_NAME_MAPPINGS_FILE, table_name_mappings):
            return {
                "status": "success",
                "message": f"表名映射保存成功: {chinese_name} -> {english_name}",
                "chinese_name": chinese_name,
                "english_name": english_name
            }
        else:
            raise HTTPException(status_code=500, detail="保存表名映射失败")
    except Exception as e:
        print(f"保存表名映射失败: {e}")
        raise HTTPException(status_code=500, detail=f"保存表名映射失败: {str(e)}")
...
async def get_field_config(table_name: str):
    ...
    try:
        ...
        return {
            "status": "success",
            "table_name": table_name,
            "chinese_name": table_config.get("chinese_name", ""),
...
@router.put("/{table_name}/field-config")
async def update_field_config(table_name: str, data: Dict[str, Any]):
    """更新表的字段配置"""
    try:
        config = read_json_file(SCHEMA_FILE)
        tables = config.get("tables", {})
        ...
        if not table_config:
            ...
            # 更新配置
            tables[table_key]["fields"] = data.get("fields", [])
            if "chinese_name" in data:
                tables[table_key]["chinese_name"] = data["chinese_name"]
        ...
        if write_json_file(SCHEMA_FILE, config):
            ...
            raise HTTPException(status_code=500, detail="保存字段配置失败")
    except Exception as e:
        print(f"更新字段配置失败: {e}")
        raise HTTPException(status_code=500, detail=f"更新字段配置失败: {str(e)}")


@router.delete("/{table_name}/field-config")
async def delete_field_config(table_name: str):
    """删除表的字段配置"""
    try:
        config = read_json_file(SCHEMA_FILE)
        tables = config.get("tables", {})
        ...
        if write_json_file(SCHEMA_FILE, config):
            ...
            raise HTTPException(status_code=500, detail="删除字段配置失败")
    except Exception as e:
        print(f"删除字段配置失败: {e}")
        raise HTTPException(status_code=500, detail=f"删除字段配置失败: {str(e)}")


@router.get("/all-field-configs")
async def get_all_field_configs():
    ...
    try:
        ...
        print(f"获取所有字段配置失败: {e}")
        raise HTTPException(status_code=500, detail=f"获取所有字段配置失败: {str(e)}")
