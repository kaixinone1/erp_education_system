...
class UniversalTemplateEngine:
    ...
    def _preview_from_excel(self, excel_path, config, query_params=None):
        ...
        # 合并单元格（先读取Excel模板中的，再合并config中动态添加的）
        merge_map = {}
        ...
        # 合并config中动态添加的合并单元格（如备注行A28:E28）
        for mc in config.get('合并单元格', []) or []:
            sr = mc.get('起始行')
            sc = mc.get('起始列')
            er = mc.get('结束行')
            ec = mc.get('结束列')
        ...
        for r in range(1, max_row + 1):
            rd = ws.row_dimensions.get(r)
            rh = (rd.height or 20) if rd else 20
            # 检查该行是否有备注等多行内容，自动增加行高
            for c_check in range(1, max_col + 1):
                filled = filled_map.get(f"{r}-{c_check}", {})
                cv_check = str(filled.get('显示值', '') or '')
                if '\n' in cv_check:
                    行数 = cv_check.count('\n') + 1
                    rh = max(rh, 行数 * 15)
                    break
...
{table_html}
    ...
    def fill_template_data(self, config, query_params):
        ...
        if not config.get('字段映射') and '绩效工资审批' not in 模板名称:
        ...
        if '绩效工资审批' in 模板名称:
            ...
            try:
                ...
                """)
                row = cur3.fetchone()
                retired_cadre_count = int(row[0] or 0) if row else 0
                ...
                """)
                row = cur3.fetchone()
                retired_worker_count = int(row[0] or 0) if row else 0
                
                cur3.execute('SELECT COUNT(*) FROM teacher_basic_info WHERE "任职状态" = \'离休\'')
                row = cur3.fetchone()
                retired_cadre_office_count = int(row[0] or 0) if row else 0
                
                print(f"退休干部: {retired_cadre_count}, 退休工人: {retired_worker_count}, 离休干部: {retired_cadre_office_count}")
                
                # 2. 查询乡镇补贴人数和标准
                cur3.execute("SELECT id FROM personal_dict_dictionary WHERE biao_qian = '绩效工资'")
                performance_tag = cur3.fetchone()
                cur3.execute("SELECT id FROM personal_dict_dictionary WHERE biao_qian = '乡镇补贴'")
                subsidy_tag = cur3.fetchone()
                ...
                if performance_tag and subsidy_tag:
                    ...
                    """, (performance_tag[0], subsidy_tag[0]))
                    row = cur3.fetchone()
                    subsidy_count = int(row[0] or 0) if row else 0
                
                subsidy_standard = 350
                cur3.execute("SELECT subsidy_amount FROM town_subsidy_standards WHERE is_active = true LIMIT 1")
                row = cur3.fetchone()
                if row and row[0]:
                    subsidy_standard = float(row[0])
                
                subsidy_total = subsidy_count * subsidy_standard
                print(f"乡镇补贴人数: {subsidy_count}, 标准: {subsidy_standard}, 合计: {subsidy_total}")
                
                # 3. 查询绩效工资汇总数据（有绩效标签的人员）
                cur3.execute("""
                ...
                """)
                row = cur3.fetchone()
                performance_count = int(row[0] or 0) if row else 0
                
                # 从岗位信息统计绩效工资总额
                performance_total = 0.0
                cur3.execute("SELECT post_1, month_performance_salary FROM dict_salary_dictionary")
                salary_standards = {}
                for r in cur3.fetchall():
                    if r[1]:
                        try:
                            salary_standards[r[0]] = float(r[1])
                        except (ValueError, TypeError):
                            pass
                
                cur3.execute("SELECT id, post FROM dict_dictionary_personal")
                dict_mapping = {}
                for r in cur3.fetchall():
                    dict_mapping[r[0]] = r[1]
                
                perf_tag_employees = set()
                cur3.execute("""
                ...
                for r in cur3.fetchall():
                    if r[0]:
                        perf_tag_employees.add(r[0])
                
                if perf_tag_employees:
                    perf_id_list = ','.join([str(x) for x in perf_tag_employees])
                    cur3.execute(f"""
                        SELECT p.post_1, COUNT(*) as cnt
                        FROM post_appointment_info p
                        WHERE p.id_card IN (
                            SELECT "身份证号码" FROM teacher_basic_info 
                            WHERE id IN ({perf_id_list})
                        )
                        AND p.post_1 IS NOT NULL
                        GROUP BY p.post_1
                    """)
                    for r in cur3.fetchall():
                        post_id = int(r[0]) if r[0] else None
                        cnt = r[1]
                        if post_id:
                            post_name = dict_mapping.get(post_id, f'未知岗位{post_id}')
                            std = salary_standards.get(post_name, 0)
                            performance_total += cnt * std
                
                print(f"绩效人数: {performance_count}, 绩效金额: {performance_total}")
                
                cur3.close()
                conn3.close()
                
                print(f"[DEBUG] retired_cadre_count={retired_cadre_count}, retired_worker_count={retired_worker_count}, retired_cadre_office_count={retired_cadre_office_count}")
                print(f"[DEBUG] subsidy_count={subsidy_count}, subsidy_standard={subsidy_standard}, subsidy_total={subsidy_total}")
                print(f"[DEBUG] performance_count={performance_count}, performance_total={performance_total}")
                
                # 4. 填充数据到对应单元格
                filled_cells = []
                for cell in filled_config.get('单元格数据', []):
                    r, c = cell.get('行号'), cell.get('列号')
                    # 退休人员（人数填入列2，列1保留原项目名称）
                    if r == 25 and c == 2:
                        cell['显示值'] = str(retired_cadre_count)
                        cell['值'] = str(retired_cadre_count)
                    elif r == 26 and c == 2:
                        cell['显示值'] = str(retired_worker_count)
                        cell['值'] = str(retired_worker_count)
                    elif r == 27 and c == 2:
                        cell['显示值'] = str(retired_cadre_office_count)
                        cell['值'] = str(retired_cadre_office_count)
                    # 绩效汇总
                    elif r == 20 and c == 2:
                        cell['显示值'] = str(performance_count)
                        cell['值'] = str(performance_count)
                    elif r == 20 and c == 4:
                        perf_total_str = str(int(performance_total)) if performance_total == int(performance_total) else str(performance_total)
                        cell['显示值'] = perf_total_str
                        cell['值'] = perf_total_str
                    # 乡镇补贴
                    elif r == 21 and c == 2:
                        cell['显示值'] = str(subsidy_count)
                        cell['值'] = str(subsidy_count)
                    elif r == 21 and c == 4:
                        sub_total_str = str(int(subsidy_total)) if subsidy_total == int(subsidy_total) else str(subsidy_total)
                        cell['显示值'] = sub_total_str
            ...
            try:
                ...
                if 绩效人数 is not None and 绩效金额 is not None:
                    ...
                    try:
                        ...
                        cur2 = conn2.cursor()
                        cur2.execute("""
                            SELECT t."姓名" FROM teacher_basic_info t
    ...
    def _get_scope_id_cards(self, cursor, config):
        ...
        sql = f'SELECT DISTINCT t."身份证号码" FROM teacher_basic_info t WHERE {where_clause}'
    ...
    def export_to_excel(self, config, output_path):
        ...
        if '页面设置' in config:
            ...
            # 还原适应页面设置：适应页宽 和 适应页高
            # 这决定了是否将内容缩放到指定页数（如适应页宽=1,适应页高=2 表示缩放到1页宽2页高）
            if '适应页宽' in page_setup and page_setup['适应页宽'] is not None:
                ws.page_setup.fitToWidth = page_setup['适应页宽']
            if '适应页高' in page_setup and page_setup['适应页高'] is not None:
                ws.page_setup.fitToHeight = page_setup['适应页高']
    ...
    def load_template_config(self, template_id):
        ...
        try:
            ...
            cursor.execute("""
                SELECT 配置json, 原始文件路径, 模板名称 FROM template_configs WHERE 模板id = %s
            """, (actual_id,))
            ...
            if row:
                ...
                # 合并数据库字段到配置中，方便下游使用
                if file_path:
                    # 如果数据库存的是相对路径，从JSON中取完整路径；否则保持JSON中的完整路径不变
                    if not os.path.isabs(file_path):
                        # 数据库存的是文件名，不覆盖JSON中的完整路径
                        pass
                    else:
                        config['原始文件路径'] = file_path
                config['模板名称'] = template_name
                return config
            return None
        finally:
            cursor.close()
